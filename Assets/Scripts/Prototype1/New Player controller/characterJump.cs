...
//You can transform the number on screen to the stat here, using the function at the bottom of this script
    ...
    [SerializeField, Range(0f, 60f)][Tooltip("Clamp the launch speed so accidental stat combos don't catapult the character. 0 disables the clamp.")] public float maxJumpLaunchSpeed = 25f;
    ...
    private void setPhysics()
    {
        ...
        float apexTime = Mathf.Max(timeToJumpApex, 0.01f);
        Vector2 newGravity = new Vector2(0, (-2 * jumpHeight) / (apexTime * apexTime));
    ...
    private void DoAJump()
    {
        ...
        if (onGround || (coyoteTimeCounter > 0.03f && coyoteTimeCounter < coyoteTime) || canJumpAgain)
        {
            ...
            if (maxJumpLaunchSpeed > 0f)
            {
                jumpSpeed = Mathf.Min(jumpSpeed, maxJumpLaunchSpeed);
            }
