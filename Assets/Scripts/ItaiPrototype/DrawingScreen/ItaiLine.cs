...
namespace ItaiPrototype.DrawingScreen
{
    public class ItaiLine : MonoBehaviour
    {
        ...
        private bool ShouldProcessCollision(Collision2D collision)
        {
            // 1. CHECK INTENSITY
            // Instead of checking "How fast am I moving?", we check "How hard was the hit?"
            // collision.relativeVelocity includes both linear speed AND rotation speed.
            float impactSqrMagnitude = collision.relativeVelocity.sqrMagnitude;

            if (impactSqrMagnitude < MinVelocityThresholdSqr)
            {
                // Hit was too soft
                return false;
            }

            // 2. PREVENT DOUBLE AUDIO (Line-vs-Line)
            if (!collision.gameObject.CompareTag("Line")) return true;
            // THE PROBLEM: In a collision, 'relativeVelocity' is identical for both objects.
            // We can no longer say "If I am slower...", because the physics engine reports 
            // the same impact speed to both parties.
        
            // THE SOLUTION: Use a unique "Tie-Breaker". 
            // We compare Instance IDs. This is arbitrary but consistent. 
            // One ID will always be lower than the other.
        
            int myID = gameObject.GetInstanceID();
            int otherID = collision.gameObject.GetInstanceID();

            // If my ID is smaller, I yield. The other line will play the sound.
            return myID >= otherID;
        }
        ...
        private float CalculateVolume(float impactSpeed)
        {
            ...
            float massFactor = Mathf.Clamp01(_cachedMass / MaxVolumeMass);
